...
CREATE                         
    PROCEDURE [dbo].[InsertBatchUploadDebitSales] (     
	@batchId int,
    @json nvarchar(max)   ,          
...
insert into ImportBatchData(          
  uniqueidentifier,Processed,WHTApplicable,VATDeffered,RCMApplicable,Isapportionment,          
    InvoiceType,          
    IRNNo,          
    InvoiceNumber,          
    IssueDate,          
...
select           
   isnull(xml_uuid, NEWID()),0,0,0,0,0,          
          'Debit Note - '+ ISNULL(InvoiceType,'') as InvoiceType,          
  IRNNo,          
