...
CREATE          procedure [dbo].[VI_VATReportStandardRatedSales15DetailedAdjustment]   -- exec VI_VATReportStandardRatedSales15DetailedAdjustment '2023-10-11', '2023-11-30',127  
(
...
INSERT INTO @temp 
 ...
 CAST(IRNNO AS INT) AS InvoiceNumber,
	...
	isnull((case when invoicetype like 'Credit Note%' and (Transtype ='Sales' or Transtype = 'AdvanceReceipt') and 
	(orignalsupplydate is null or orignalSupplydate < @fromdate)
        then (isnull(LineNetAmount,0))
        else 0 end),0) - 
	isnull(
    (case when invoicetype like 'Debit Note%' and (Transtype ='Sales' or Transtype = 'AdvanceReceipt') and 
	(orignalsupplydate is null or orignalSupplydate < @fromdate)
        then (isnull(LineNetAmount,0))
        else 0
