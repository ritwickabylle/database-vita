...
SET ANSI_NULLS ON
GO
CREATE     procedure [dbo].[VI_VATReportImportSubjecttoRCM5]   
(  
@fromdate date,  
@todate date,
@tenantId int=null
...
select 14,'9.1 Imports subject to VAT accounted for through reverse charge mechanism 5%',isnull(sum((case when (invoicetype like 'Purchase Entry%')   
...
--and  left(BuyerCountryCode,2) <> 'SA' and VatRate=5  and (VATDeffered =1 or RCMApplicable =1)    -- commented on 05-mar-2024
and  VatRate=5  and (VATDeffered =1 or RCMApplicable =1)    -- added on 05-mar-2024
