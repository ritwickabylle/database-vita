...
CREATE     PROCEDURE [dbo].[GetDataCIT_Schedule17]    -- exec [GetDataCIT_Schedule17] 148, '1/1/2023', '12/31/2023'
(@cols nvarchar(max) =N'',
@tenantId INT =148,
...
set @query = N'
 SELECT       
   [UniqueIdentifier] as uniqueidentifier_column,
	...
	[Reasonfordeductionfromzakatbase] as ''Reason for deduction from zakat base'',
    [Amount] as ''Amount''
   FROM [dbo].[CIT_Schedule17] where isActive = 1 and Tenantid = @tenantId       
