...
SET ANSI_NULLS ON
GO
CREATE       procedure [dbo].[GetSalesDetailedReportNominalTaxValue]   -- exec GetSalesDetailedReportNominalTaxValue '2022-12-01', '2022-12-30',19,'VATSAL004'                    
(                    
@fromDate Date=null,                    
@toDate Date=null,                  
@tenantId int=null,              
...
BuyerName as CustomerName,
 ...
 where CAST(IssueDate AS DATE)>=@fromDate and CAST(IssueDate AS DATE)<=@toDate and InvoiceType like 'Sales%'  and InvoiceType like '%Nominal%'    
 and upper(VatCategoryCode) like 'O' and NetPrice < 200    
 --and orignalSupplyDate < @fromDate              
AND TenantId=@tenantId          
group by IRNNo,effdate,InvoiceNumber,BillingReferenceId,VatRate,BuyerName order by VatRate  
