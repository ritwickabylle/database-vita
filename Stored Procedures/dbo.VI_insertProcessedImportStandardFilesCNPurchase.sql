...
SET ANSI_NULLS ON
GO
CREATE       procedure [dbo].[VI_insertProcessedImportStandardFilesCNPurchase]      --  
(          
@batchno numeric  ,        
@tenantid int        
)          
...
and uniqueIdentifier in (select uniqueIdentifier from ##salesImportBatchDataTemp where Batchid=@batchno and upper(trim(InvoiceType)) like 'CN Purchase%')      
    
delete from importStandardfiles_ErrorLists where batchid = @batchno and tenantid = @tenantid and uniqueIdentifier in (select uniqueIdentifier from ##salesImportBatchDataTemp where Batchid=@batchno and upper(trim(InvoiceType)) like 'CN Purchase%')      
...
where upper(trim(I.invoicetype)) like  'CN Purchase%' and i.batchid = @batchno and tenantid = @tenantid and i.UniqueIdentifier not in           
(select UniqueIdentifier from ImportStandardFiles_Errorlists e where e.batchid = @batchno and tenantid = @tenantid)      
    
delete from VI_importstandardfiles_processed where  batchid = @batchno  and   upper(trim(invoicetype)) like 'CN Purchase%'        
