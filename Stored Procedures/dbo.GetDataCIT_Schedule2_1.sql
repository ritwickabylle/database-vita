...
CREATE       PROCEDURE [dbo].[GetDataCIT_Schedule2_1]    -- exec [GetDataCIT_Schedule2_1] 159, '1/1/2023', '12/31/2023'    
...
set @query = N'  
 ...
 SELECT     
   [UniqueIdentifier] as uniqueidentifier_column, ROW_NUMBER() over (order by id ) as  [sno.] ,  [Description] as ''Description'',[Amount] as ''Amount''   
   FROM [dbo].[CIT_Schedule2_1] where isActive = 1 and Tenantid = @tenantId     
