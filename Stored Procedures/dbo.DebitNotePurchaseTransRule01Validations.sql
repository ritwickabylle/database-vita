...
where InvoiceType  like 'DN Purchase%' and  upper(trim(RIGHT(invoicetype , LEN(invoicetype) - CHARINDEX('-', invoicetype) ))) like 'Import%' and upper(trim(PurchaseCategory)) like 'GOOD%'
--and (BillOfEntry is null or len(BillOfEntry)=0 or BillOfEntryDate is null or len(BillOfEntryDate)=
--0)   
