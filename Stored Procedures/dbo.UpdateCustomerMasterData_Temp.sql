...
SET ANSI_NULLS ON
GO
CREATE      procedure [dbo].[UpdateCustomerMasterData_Temp]   -- exec updatecustomermasterdata_Temp 137,2  
(  
 @batchno numeric,  
 @tenantid numeric  
)  
...
--select * from CustomerDocuments --check
--select * from CustomerForeignEntity --issue
...
and i.MasterType like 'Customer%' and customers.id = i.masterid
...
begin
insert into customers(UniqueIdentifier,TenantId,ConstitutionType,name,LegalName,ContactPerson,ContactNumber,EmailID,Nationality,  
...
i.masterid not in (select id from customers where tenantid = @tenantid) --and concat()
...
and c.ConstitutionType=VI_ImportMasterFiles_Processed.ConstitutionType order by c.id desc) where VI_ImportMasterFiles_Processed.batchid=@batchno
...
and c.ConstitutionType=ImportMasterBatchData.ConstitutionType order by c.id desc) where ImportMasterBatchData.batchid=@batchno
end

--select c.id,i.masterid from  customers c inner join ImportMasterBatchData i on c.legalname=i.LegalName and c.Name=i.Name and c.ConstitutionType=i.ConstitutionType and i.batchid=131
...
--else  
begin   
 insert into CustomerDocuments (TenantId,UniqueIdentifier,CustomerID,CustomerUniqueIdentifier,DocumentTypeCode,DocumentNumber,doumentdate,CreationTime,CreatorUserId,IsDeleted)  
 select TenantId,NEWID(),MasterId,newid(),DocumentType,DocumentNumber,getdate(),CreationTime,CreatorUserId,IsDeleted  
 from VI_ImportMasterFiles_Processed i  
...
and CustomerForeignEntity.TenantId=@tenantid and i.ConstitutionType like 'Foriegn%'
...
--else  
begin   
 insert into CustomerTaxDetails (TenantId,UniqueIdentifier,CustomerID,CustomerUniqueIdentifier,SalesVATCategory,CreationTime,CreatorUserId,IsDeleted)  --SalesVATCategory
 select TenantId,NEWID(),masterid,newid(),SalesVATCategory,CreationTime,CreatorUserId,IsDeleted  
 from VI_ImportMasterFiles_Processed i  
 where i.TenantId=@tenantid and i.batchid=@batchno 
...
begin   
 ...
 from VI_ImportMasterFiles_Processed i  
 where i.TenantId=@tenantid and i.batchid=@batchno 
 and BusinessCategory not in (select isnull(BusinessCategory,'') from CustomerTaxDetails where CustomerID=91 and TenantId=2)
end  
