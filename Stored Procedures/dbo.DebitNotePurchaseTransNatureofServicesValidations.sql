...
select tenantid,@batchno,uniqueidentifier,'0','Invalid Nature of Services',573,0,getdate() from ##salesImportBatchDataTemp                   
...
insert into importstandardfiles_errorlists(tenantid,BatchId,uniqueidentifier,Status,ErrorMessage,Errortype,isdeleted,CreationTime)                   
select tenantid,@batchno,uniqueidentifier,'0','if WHT Applicable ,Nature of Services Required ',574,0,getdate() from ##salesImportBatchDataTemp                   
where Invoicetype like 'DN Purchase%' and whtapplicable like 'Y%'                
