...
CREATE         PROCEDURE [dbo].[GetDebitData]         -- exec GetDebitData '2023-11-09','2023-11-22',140 ,null,null,null,null,null,null,null                            
@fromDate DATETIME,                            
@toDate DATETIME,                                                
@tenantId INT,                              
...
@IRNo NVARCHAR(MAX) = NULL,@createdby nvarchar(max)=NULL                        
...
(select sap_dn_number from openjson(s.additionaldata2) with (sap_dn_number nvarchar(max) '$.sap_dn_number')) as  Debit_Memo_SAP_Nr,                
                  
(select CNRefDate from openjson(s.additionaldata2) with (CNRefDate nvarchar(max) '$.sap_dn_date')) as  SAP_DN_Reference_Date,                              
(select sap_invoice_date from openjson(s.additionaldata2) with (sap_invoice_date nvarchar(max) '$.sap_invoice_date')) as  SAP_Invoice_Date,                   
(select original_order_number from openjson(s.additionaldata2) with (original_order_number nvarchar(max) '$.original_order_number')) as  original_order_number,                  
...
s.IRNNo as Debit_Note_Number,                        
s.IRNNo as Invoice_Number,                        
cast(format(s.IssueDate,'dd-MM-yyyy') as nvarchar(max)) as Debit_Note_Date,

case when s.Source = 'FTP' then
(select ISNULL(salesorderno,'') from openjson(s.additionaldata2) with (salesorderno nvarchar(max) '$.dn_order_number')) 
else 
(select ISNULL(salesorderno,'') from openjson(s.additionaldata2) with (salesorderno nvarchar(max) '$.dn_order_number')) end
as Sales_Order_Number, 

case when s.Source = 'FTP' then
(select ISNULL(purchaseorderno,'') from openjson(s.additionaldata2) with (purchaseorderno nvarchar(max) '$.dn_po_number'))
else
(select ISNULL(purchaseorderno,'') from openjson(s.additionaldata2) with (purchaseorderno nvarchar(max) '$.dn_po_number')) end
...
FROM                                               
  ...
  FROM DebitNoteItem   WITH (NOLOCK)  where isOtherCharges = 0                                         
  GROUP BY IRNNo,TenantId) i ON s.IRNNo = i.IRNNo  AND ISNULL(i.tenantid,0)=ISNULL(@tenantId,0) AND ISNULL(s.tenantid,0)=ISNULL(@tenantId,0)                        
   left JOIN (SELECT SUM(LineAmountInclusiveVAT) AS LineAmountInclusiveVAT,IRNNo,TenantId   FROM DebitNoteItem   WITH (NOLOCK) where isOtherCharges = 1                        
GROUP BY IRNNo,TenantId) ii ON s.IRNNo = ii.IRNNo  AND ISNULL(ii.tenantid,0)=ISNULL(@tenantId,0) AND ISNULL(s.tenantid,0)=ISNULL(@tenantId,0)                        
  ...
  inner join DebitNoteParty su with (nolock) on s.IRNNo = su.IRNNo   and Isnull(su.tenantid,0)=isnull(@tenantId,0)                                            
...
as Purchase_Order_Number,                    
 ...
 s.CreationTime                        
    from draft s                        
 INNER JOIN (SELECT SUM(LineAmountInclusiveVAT) AS LineAmountInclusiveVAT,IRNNo,TenantId,max(Vatrate) as VatRate                           
FROM draftitem   WITH (NOLOCK) where isOtherCharges = 0 and IsDeleted=0    
...
where CAST(s.IssueDate AS DATE)>=CAST(@fromDate as Date) and CAST(s.IssueDate AS DATE)<= CAST(@toDate as Date) and s.source in ('UI','API') and s.isSent=0 and s.InvoiceTypeCode='383')                        
) as t1;                        
  ...
  BEGIN 
  DELETE FROM #invoice WHERE dbo.Dateconverter(SAP_DN_Reference_Date) not LIKE '%'+ dbo.Dateconverter(cast(format(@creationDate,'yyyy-MM-dd') as nvarchar(30)))+'%' or SAP_DN_Reference_Date is null; 
  ...
  BEGIN                            
  DELETE FROM #invoice WHERE Debit_Memo_SAP_Nr <> @invoicerefno or Debit_Memo_SAP_Nr is null;                            
  END                            
  ...
  END                            
   ...
   IF(@IRNo IS NOT NULL)                            
  BEGIN                            
  DELETE FROM #invoice WHERE Debit_Note_Number IS NOT NULL and Debit_Note_Number NOT LIKE '%'+@IRNo+'%';                            
  END                           
...
begin                          
SELECT Debit_Memo_SAP_Nr as SAP_DN_Nr ,dbo.dateconverter(SAP_DN_Reference_Date) as SAP_DN_Date,Customer_Name,format(cast(Amount as decimal(20,2)) ,'#,0.00')as [Amount_(total)],  
Currency,pdf,Invoice_Number, [Status],Sales_Order_Number,Purchase_Order_Number,Payer_Number,Ship_To_Number,Debit_Note_Number,Debit_Note_Date                  
,Contact_Details,Source,Created_By,format(CreationTime,'dd-MM-yyyy HH:MM') as Creation_Time          
FROM #invoice  ORDER BY CreationTime desc                          
end                          
else                           
begin                          
SELECT Debit_Note_Number,CONVERT(VARCHAR(10), Debit_Note_Date, 101) as Debit_Note_Date,pdf,Invoice_Number, Customer_Name,[Status],Contact_Details,Currency,format(cast(Amount as decimal(20,2)),'#,0.00') as Amount,Invoice_Reference_Number From #invoice  ORDER BY CreationTime desc                      
