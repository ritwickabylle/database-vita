...
SET ANSI_NULLS ON
GO
CREATE      procedure [dbo].[VI_insertProcessedImportStandardFilesDN]           --exec VI_insertProcessedImportStandardFilesDN 659            
(                      
@batchno numeric ,        
@tenantid int        
)                      
...
where upper(trim(I.invoicetype)) like 'DEBIT%' or upper(trim(I.invoicetype)) like 'DN%' and i.batchid = @batchno and tenantid = @tenantid and i.UniqueIdentifier not in                   
(select UniqueIdentifier from ImportStandardFiles_Errorlists e where e.batchid = @batchno and tenantid = @tenantid)                          
                       
delete from VI_importstandardfiles_processed where  batchid = @batchno  and TenantId=@tenantid   and   upper(trim(invoicetype)) like 'DEBIT%'                    
