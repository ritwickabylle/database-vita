...
CREATE         procedure [dbo].[GetDebitNotePeriodicalReportIssuedate]    -- exec   GetDebitNotePeriodicalReportIssuedate '2023-10-03','2023-10-03',2140,'any',null
(    
@fromDate DATE=NULL,          
@toDate DATE=NULL,        
@tenantId INT=NULL,    
...
group by IRNNo,IssueDate,InvoiceNumber,BuyerName,BillingReferenceId,VatRate,BillOfEntryDate,ContractId,PurchaseOrderId,BuyerMasterCode,BillOfEntry,VatCategoryCode,invoicetype  ) debit WHERE    
    (    
        CASE    
...
CAST(IssueDate AS DATE)>=@fromDate and CAST(IssueDate AS DATE)<=@toDate        
and invoicetype like 'Debit Note%' and VatCategoryCode not like '0'           
group by IRNNo,IssueDate,InvoiceNumber,BuyerName,BillingReferenceId,VatRate,BillOfEntryDate,ContractId,PurchaseOrderId,BuyerMasterCode,BillOfEntry  ) debit where    
