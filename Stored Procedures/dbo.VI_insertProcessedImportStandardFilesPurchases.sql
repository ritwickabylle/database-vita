...
CREATE        procedure [dbo].[VI_insertProcessedImportStandardFilesPurchases]           
(          
@batchno numeric,      
@tenantid int      
)          
...
and uniqueIdentifier in (select uniqueIdentifier from ##salesImportBatchDataTemp where Batchid=@batchno and upper(trim(InvoiceType)) like 'PURCHASE%')    
        
      
delete from importStandardfiles_ErrorLists where batchid = @batchno and tenantid = @tenantid and uniqueIdentifier in (select uniqueIdentifier from ##salesImportBatchDataTemp where Batchid=@batchno and upper(trim(InvoiceType)) like 'PURCHASE%')    
and uniqueIdentifier in (select uniqueIdentifier from Transactionoverride where batchid = @batchno and tenantid=@tenantid)      
          
insert into importstandardfiles_errorlists(tenantid,BatchId,uniqueidentifier,Status,ErrorMessage,Errortype,isdeleted,CreationTime)           
          
select i.tenantid,@batchno,i.uniqueidentifier,'1',' ',0,0,getdate() from ##salesImportBatchDataTemp i          
where upper(trim(I.invoicetype)) like 'PURCHASE%' and i.batchid = @batchno and tenantid = @tenantid and i.UniqueIdentifier not in           
(select UniqueIdentifier from ImportStandardFiles_Errorlists e where e.batchid = @batchno and tenantid = @tenantid)           
           
delete from VI_importstandardfiles_processed where  batchid = @batchno and tenantid = @tenantid and   upper(trim(invoicetype)) like 'PURCHASE%'        
...
INSERT INTO [VI_importstandardfiles_Processed]          
     ...
     from ##salesImportBatchDataTemp i inner join importstandardfiles_ErrorLists e on i.UniqueIdentifier = e.uniqueIdentifier and i.Batchid=@batchno and e.status = '1' and i.TenantId=@tenantid and e.TenantId=@tenantid         
        
--  update VI_importstandardfiles_Processed set effdate = (case when IssueDate < SupplyDate then issuedate else SupplyDate end) where batchid = @batchno and issuedate is not null and supplydate is not null      
   update vi_importstandardfiles_processed set effdate = issuedate where batchid = @batchno  and TenantId=@tenantid    
    
exec VI_insertProcessedImportOverheadFilesPurchases @batchno,@tenantid     
