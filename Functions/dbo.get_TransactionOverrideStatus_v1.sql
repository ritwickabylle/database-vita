...
CREATE     FUNCTION [dbo].[get_TransactionOverrideStatus_v1] (@uuid uniqueidentifier,@tenantid int)   -- exec get_transactionoverridestatus '1CC6DA24-5EB2-4AEA-BA25-E2A963FE0D11'  
RETURNS int AS  
BEGIN  
...
    if(@countAll =         
    (select  count(errortype) from importstandardfiles_ErrorLists with(nolock) where   
    uniqueIdentifier =@uuid and   TenantId=@tenantid and
